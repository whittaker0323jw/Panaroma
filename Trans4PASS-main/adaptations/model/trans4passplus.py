...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768, use_dcn=False):
        ...
        if use_dcn:
            self.offset_conv = nn.Conv2d(in_chans,
                                         ...
                                         stride=stride,
                                         padding=self.padding)#输入通道数，输出通道数
            nn.init.constant_(self.offset_conv.weight, 0.)
            nn.init.constant_(self.offset_conv.bias, 0.)
            self.modulator_conv = nn.Conv2d(in_chans,
                                            1 * patch_size[0] * patch_size[0],#输出通道数
                                            kernel_size=self.kernel_size,
                                            stride=stride,
    ...
    def deform_proj(self, x):
        # h, w = x.shape[2:]
        max_offset = min(x.shape[-2], x.shape[-1]) // 4 #r=4
        offset = self.offset_conv(x).clamp(-max_offset, max_offset)#对输入进行offset计算，并且限制范围
        ###########################修改#############################################################
        offset.zero_()#将offset里面的值都设为0
