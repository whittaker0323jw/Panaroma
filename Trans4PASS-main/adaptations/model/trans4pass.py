...
class OverlapPatchEmbed(nn.Module):
    ...
    def deform_proj(self, x):
        ...
        # offset = torch.load('offset_tensor_400x2048.pt').to('cuda')
        # B = x.shape[0]
        # offset = offset.repeat(B,1,1,1)
        # print("Offset shape0:", offset.shape)
        # offset = self.offset_conv(x).clamp(-max_offset, max_offset)
        # offset = torch.load('offset_tensor_1024x2048.pt').to('cuda')
        # B = x.shape[0]
        # offset = offset.repeat(B,1,1,1)
        # print("输入张量的数据类型:", x.dtype) #因为运行了四次
        # print("x的大小",x.shape[2:])
        # print("Offset shape:", offset.shape[3])#Offset shape: torch.Size([1, 98, 100, 512])
        # print("Offset_x_test:",offset[0,:,0,0])
        # copied_tensor = tensor.repeat(4, 1, 1, 1)
        offset = self.offset_conv(x).clamp(-max_offset, max_offset)
        offset.fill_(1)
        if(offset.shape[2]== 100):
            offset = torch.load('offset_tensor_400x2048.pt').to('cuda')
            B = x.shape[0]
            offset = offset.repeat(B,1,1,1)
            print("Offset shape0:", offset.shape)
        if(offset.shape[2]== 256):
            
            offset = torch.load('offset_tensor_1024x2048.pt').to('cuda')
            B = x.shape[0]
            offset = offset.repeat(B,1,1,1)
        if(offset.shape[2] == 128):
            offset = torch.load('offset_tensor_512x1024.pt').to('cuda')
            B = x.shape[0]
            offset = offset.repeat(B,1,1,1)
        self.register_buffer('offset', offset)
        # print("Offset shape:", offset.shape)
        modulator = 2. * torch.sigmoid(self.modulator_conv(x))
        # modulator = torch.ones_like(modulator)
