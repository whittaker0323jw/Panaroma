...
class PANODataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(self.ssl_dir)>0:
            label = Image.open(osp.join(self.ssl_dir, name.replace('.jpg', '_labelTrainIds.jpg')))
            if self.trans == 'resize':
                # resize
...
##no test set for pano, currently no need to change 
class PANOTestDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(2048, 400), mean=(128, 128, 128),
        ...
        for name in self.img_ids:
            ...
            #try not use gtFine(label) foler(cuz we dunt have one@.@)
...
if __name__ == '__main__':
    dst = PANOTestDataSet("data/DensePASS_train_pseudo_val", 'dataset/densepass_list/val.txt', mean=(0,0,0))
