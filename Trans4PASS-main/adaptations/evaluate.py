...
SAVE_PATH = './result_temp/' + DIR_NAME
...
def main():
    ...
    for index, batch in enumerate(testloader):
        ...
        for sc in scales:
            ...
            # print("img_shape:", img_tem.shape)# torch.Size([1, 3, 400, 2048])
            with torch.no_grad():
                ...
                # output_temp += interp((output_rot2+output_rot1+output_rot0+output)/4) #new added
                output_temp += interp(output)
        ...
        # output_rot = output_temp_rot / len(scales)
        # output_new = torch.argmax((output+output_rot)/2, 1).squeeze(0).cpu().data.numpy()
        ...
        if save_vis:
            ...
            # output_col = colorize_mask(label) #输出label对应的colormap
