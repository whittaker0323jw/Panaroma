...
from model.discriminator import FCDiscriminator,FCDiscriminator_norm
from dataset.cs_dataset_src import CSSrcDataSet
from dataset.densepass_dataset import densepassDataSet, densepassTestDataSet#需要改
from dataset.PANO_dataset import PANODataSet
#from dataset.PANO_dataset import densepassDataSet, densepassTestDataSet
...
logging.basicConfig(filename='/Downloads/Trans4PASS-main/adaptations/logfile.log', level=logging.INFO)
...
SOURCE_NAME = 'CS'
# TARGET_NAME = 'DensePASS'#需要改
TARGET_NAME = 'PANO'
DATA_DIRECTORY = 'datasets/cityscapes'
DATA_LIST_PATH = 'dataset/cityscapes_list/train.txt'
IGNORE_LABEL = 255
INPUT_SIZE = '1024,512'
# DATA_DIRECTORY_TARGET = 'datasets/DensePASS' #需要改
DATA_DIRECTORY_TARGET = 'datasets/PANO'
# DATA_LIST_PATH_TARGET = 'dataset/densepass_list/train.txt'#需要改
DATA_LIST_PATH_TARGET = 'dataset/PANO_list/train.txt'
DATA_LIST_PATH_TARGET_TEST = 'dataset/densepass_list/val.txt'#需要改 ?但是PANO没有val集
# INPUT_SIZE_TARGET = '2048,400'#需要改
...
RESTORE_FROM = 'snapshots_offset/CS2PANO_Trans4PASS_v1_WarmUp/BestCS2PANO_4000iter_44.09miou.pth'
SAVE_NUM_IMAGES = 2
SAVE_PRED_EVERY = 1000
# SAVE_PRED_EVERY = 1000
DIR_NAME = '{}2{}_{}_WarmUp/'.format(SOURCE_NAME, TARGET_NAME, MODEL)
SNAPSHOT_DIR = './snapshots_offset/' + DIR_NAME
...
def main():
    ...
    targetset = PANODataSet(args.data_dir_target, args.data_list_target, max_iters=args.num_steps * args.iter_size * args.batch_size,
    ...
    for i_iter in range(Iter, args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            src_feature, pred = model(images)  # src_feature = [c1, c2, c3, c4] #prediction from the model trained from source images(cityscape)

            #print("src_feature shape:", [x.shape for x in src_feature])
            # logging.info("src_feature shape: %s", [x.shape for x in src_feature])
            src_feature = src_feature[-1]
            # print("src_feature[-1] shape:", src_feature.shape)

            pred = interp(pred)
            # print("interp(pred) shape:", pred.shape)
