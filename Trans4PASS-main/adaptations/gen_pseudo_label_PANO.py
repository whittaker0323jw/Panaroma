...
RESTORE_FROM = 'snapshots/CS2DensePASS_Trans4PASS_v1_WarmUp/BestCS2DensePASS_500iter_41.58miou.pth' #调用的是总的模型（pth full）
SET = 'train'
...
def main():
    ...
    targetset = PANODataSet(args.data_dir, args.data_list, crop_size=(1024,512), set=args.set)
    ...
    predicted_label = np.zeros((len(targetset), 512,1024), dtype=np.int8)
    predicted_prob = np.zeros((len(targetset), 512,1024), dtype=np.float16)
    image_name = []
