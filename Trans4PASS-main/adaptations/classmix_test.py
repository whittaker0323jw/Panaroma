...
def main():
    ...
    targetloader_iter_patch = enumerate(targetloader)
    ...
    for i_iter in range(Iter, args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            pred_target = interp_target(pred_target)#upsample、、、
            loss_seg_trg = seg_loss_target(pred_target, trg_labels)#cross entropy loss
            ...
            #这部分代码是创建一个与 D_out 相同大小的张量，其值都被填充为 source_label=0
            #希望是target预测结果能更接近source label
            ...
            (F.softmax(pred_target, dim=1))

            loss_D = bce_loss(D_out, torch.FloatTensor(D_out.data.size()).fill_(target_label).to(device))
            loss_D = loss_D / args.iter_size / 2
            amp_backward(loss_D, optimizer_D)
            loss_D_value += loss_D.item()
            #loss_D包含两部分：source结果损失和target结果损失，分别计算。目的是训练出一个能分辨source和target的分辨器。
