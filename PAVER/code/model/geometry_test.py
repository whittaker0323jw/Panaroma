...
###############compute all patches#############################
def compute_all_patches(patch_size,model_config, is_discrete=False):
    # patch_size = model_config['patch_size']
    # resolution = model_config['input_resolution']
    patch_size_re = 4
    resolution = 512
    patch_no = resolution // patch_size_re  # 224 / 16 = 14 #这么小吗？ #256
    ...
    x, y = np.meshgrid(np.linspace(-P, P, R, dtype=np.float64), #例：（-pi）                                             
    ...
    lon_range = patch_no * 2 - 1 # 27 #此处设定上是lon = 2*lat
...
def compute_deform_offset(model_config, is_discrete=False):
    # R = model_config['patch_size']  #16
    R1=7
    R= R1-3
    patches = compute_all_patches(patch_size=R1,model_config=model_config, is_discrete=is_discrete)
    deform_offset = []
